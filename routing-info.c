...
int check_primary_route(struct nlmsghdr* nl_header_answer, char *if_name)
{
	...
	if (!tb[RTA_DST] && !r->rtm_dst_len)	// default gateway
	{
		if (tb[RTA_OIF])
		{
...
int get_route_dump_check_primary(int sock, char ifa_name[5][16], size_t if_numbers)
{
	...
	while (NLMSG_OK(h, msglen)) {
		...
		for (int i = 0; i < if_numbers; i++)
		{
			if (check_primary_route(h, ifa_name[i]))
			{
				result = i;
				break;
			}
